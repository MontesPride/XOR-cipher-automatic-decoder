...
def break_repeat_key_xor(ciphertext):
    ...
    hamming_blocks = int(len(ciphertext)*0.01)
    #hamming_blocks = 30
    key_sizes = find_xor_keysize(ciphertext, hamming_blocks , 2, 15)
    ...
    for ks in key_sizes:
        ...
        for keys in real_keys:
            ...
            if is_english(text):
                ...
                print "Key: ", keys[0]
                print "Key as bytes:", list(keys[1])
                raw_input()
                print "=================="
...
with open("crypto_xorascii.bin", "rb") as file:
    for line in file:
        break_repeat_key_xor(line)
